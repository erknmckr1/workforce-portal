    ...
    <header className="h-[73px] flex items-center justify-between px-4 lg:px-8 bg-background border-b border-border transition-all duration-300 backdrop-blur-md sticky top-0 z-30">
      ...
      <div className="flex items-center gap-2 lg:gap-4">
        ...
        <div className="relative" ref={notificationRef}>
          ...
          >
            <Bell size={22} className={cn("transition-colors", showNotifications ? "text-primary" : "group-hover:text-primary")} />
            {unreadCount > 0 && (
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-3 w-80 sm:w-96 rounded-2xl bg-card border border-border shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 z-50">
              <div className="p-4 border-b border-border/50 flex items-center justify-between bg-muted/20">
                <h3 className="font-black text-sm uppercase tracking-tighter">Bildirimler</h3>
                {unreadCount > 0 && (
                  <button 
                    onClick={() => markAllRead()}
              ...
              <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
                {notifications.length === 0 ? (
                  <div className="p-10 text-center flex flex-col items-center gap-2 opacity-50">
                    ...
                    <span className="text-xs font-bold uppercase tracking-widest">Bildirim bulunmuyor</span>
                  </div>
                ) : (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 mb-1">
                          <span className={cn("text-sm tracking-tight truncate", notif.is_read ? "font-medium text-muted-foreground" : "font-black text-foreground")}>
                            {notif.title}
                          </span>
                        ...
                        <p className={cn("text-xs leading-relaxed line-clamp-2", notif.is_read ? "text-muted-foreground/70" : "text-muted-foreground font-medium")}>
                          {notif.message}
                        </p>
                      </div>
                    </button>
              ...
              {notifications.length > 0 && (
                <div className="p-3 bg-muted/10 border-t border-border/50 text-center">
                  <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Son 20 bildirim gösteriliyor</span>
                </div>
              )}
            </div>
          )}
